...
class apfeed:
    ...
    def add_inv(self, inv):
        ...
        vend_assign_inv_date = datetime.datetime.strptime(
            inv.find("exl:invoice_date", ns).text,
            "%m/%d/%Y"
        )
        addr_select_vend_nbr = inv.find(
            "exl:vendor_additional_code", ns
        ).text.replace(" ", "")
        pmt_remit_nm = " " * 40
        ...
        if note_list is None:
            ...
            goods_received_dt = note_list.find(
                "exl:note/exl:owneredEntity/exl:creationDate", ns
            ).text
        ...
        project_cd = " " * 10
        vat_amt = float(
            inv.find("exl:vat_info/exl:vat_amount", ns).text
        )
        ...
        inv_list = inv.findall("./exl:invoice_line_list/exl:invoice_line", ns)
